// Design guidelines returned by visualize_read_me.

const CORE = `# Widget Design Guide

## Modules
Use the modules parameter to load detailed guidance:
...
- Box subtitles: ≤5 words. Put detail in the prose below — not the box.
- Colors: ≤2 ramps per diagram. If colors encode meaning (states, tiers), add a 1-line legend. Otherwise use one neutral ramp.
- Horizontal tier: ≤4 boxes at full width (~140px each). 5+ boxes → shrink to ≤110px OR wrap to 2 rows OR split into overview + detail diagrams.

If you catch yourself writing "click to learn more" in prose, the diagram itself must ACTUALLY be sparse. Don't promise brevity then front-load everything.

You create rich visual content — SVG diagrams/illustrations and HTML interactive widgets — that renders in a native widget window. The best output feels like a natural extension of the chat.
...
### Philosophy
- **Seamless**: Visuals should feel like a natural part of the chat.
...
- **Typography**: The default font is the host sans-serif. For the rare editorial/blockquote moment, use \`font-family: var(--font-serif)\`.
...
- Never use \`position: fixed\` — the widget viewport sizes itself to your in-flow content height, so fixed-positioned elements (modals, overlays, tooltips) collapse it to \`min-height: 100px\`. For modal/overlay mockups: wrap everything in a normal-flow \`<div style="min-height: 400px; background: rgba(0,0,0,0.45); display: flex; align-items: center; justify-content: center;">\` and put the modal inside — it's a faux viewport that actually contributes layout height.
...
- **Recommended CDN sources**: prefer \`cdnjs.cloudflare.com\`, \`esm.sh\`, \`cdn.jsdelivr.net\`, or \`unpkg.com\` for external libraries.
...
### Widget actions
Widgets are display-only from pi's perspective. Use JavaScript only for local widget state; do not rely on callbacks into the agent.

### Links
\`<a href="https://...">\` can be used when a link is genuinely needed, but keep widgets useful without external navigation.
...
- Use JavaScript controls only for state changes the widget can handle itself`;

const ART_AND_ILLUSTRATION = `## Art and illustration
*"Draw me a sunset" / "Create a geometric pattern"*

Use \`show_widget\`. Same technical rules (viewBox, safe area) but the aesthetic is different:
...
**Dashboard layout** — wrap summary numbers in metric cards (see UI fragment) above the chart. Chart canvas flows below without a card wrapper. Put drill-down suggestions in normal response text, not widget callbacks.`;
...
Use \`show_widget\` for diagrams. Provide a complete SVG element or HTML fragment directly.
...
The illustrative route is the default for *"how does X work"* with no further qualification. It is the more ambitious choice — don't chicken out into a flowchart because it feels safer. The assistant draws these well.
...
This is the most ambitious diagram type and the one the assistant should handle boldly. Lean into it. Use colour for intensity (a hot attention weight glows amber, a cold one stays gray). Use repetition for scale (many small circles = many parameters).
...
**Font size calibration for diagram text labels** - Here's csv table to give you better sense of the sans-serif font rendering width:
\`\`\`csv
...
- Buttons: pre-styled with transparent bg, 0.5px border-secondary, hover bg-secondary, active scale(0.98). If it acts as a local control, make the result visible inside the widget.
...
Use sliders, buttons, and inputs only for calculations or state changes the widget can perform locally.
...
- Add buttons only for local filtering, weighting, toggles, or calculations.
