import morphdomModule from 'morphdom';
...
type Morphdom = (fromNode: Node, toNode: Node | string, options?: MorphdomOptions) => Node;

const morphdom = morphdomModule as unknown as Morphdom;
  ...
  for (const old of scripts) {
    ...
    if (old.src) {
      const loaded = new Promise<void>((resolve, reject) => {
        ...
        s.addEventListener('error', () => reject(new Error(`Failed to load ${old.src}`)), {
          once: true
        });
      });
