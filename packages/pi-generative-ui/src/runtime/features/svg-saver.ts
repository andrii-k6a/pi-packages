...
function suggestName(svg: SVGElement): string {
  ...
  return svg.getAttribute('aria-label') || titleNode?.textContent || document.title || 'diagram';
}
...
export function install(): void {
  ...
  Array.from(trigger.children).forEach((dot) => {
    (dot as HTMLElement).style.cssText =
      'width:3.5px;height:3.5px;border-radius:50%;background:#c8c5bd;display:block;';
  });
  ...
  on('content', (msg) => {
    if (msg.final) setExportReady(true);
  });
  ...
  copy.item.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
    void dispatch('copy');
  });
  download.item.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
    void dispatch('download');
  });

  window.addEventListener(
    'scroll',
    () => {
      if (activeSvg) showFor(activeSvg);
    },
    true
  );
