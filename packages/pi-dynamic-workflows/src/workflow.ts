...
// narrowed by runtime `type` checks below.
// biome-ignore lint/suspicious/noExplicitAny: Acorn AST node shapes vary by node kind.
...
export async function runWorkflow<T = unknown>(
  ...
  const workflowProfile = meta.profile
    ? resolveProfile(meta.profile, options.profileResolver)
    : undefined;
  ...
  const phase = (title: unknown, phaseOptions: unknown = {}) => {
    ...
    state.currentProfile = normalizedOptions.profile
