...
curl -s "https://pypi.org/pypi/requests/json" | jq '.info.requires_python'
```
...
For this skill, the default SearXNG config lives under:

```bash
${XDG_CACHE_HOME:-$HOME/.cache}/web-discovery/settings.yml
```

Changing PyPI engine settings is unlikely to fix PyPI search while PyPI serves bot-protection challenges to scrapers. Prefer the direct alternatives above.
